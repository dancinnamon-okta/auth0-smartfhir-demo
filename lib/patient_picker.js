...
module.exports.getHandler = async (requestQueryString) => {
  ...
  console.log(verifiedAuth0Data)
	...
	if(verifiedAuth0Data.body.requested_scopes.includes('launch/patient') && !verifiedAuth0Data.body.requested_scopes.includes('skip_patient_selection')) {
		try {
...
module.exports.postHandler = async (requestBodyString) => {
  ...
  const returnUrl = 'https://' + process.env.AUTH0_ORG + '/continue?' +
                    'state=' + auth0State +
                    '&token=' + jwt
      ...
      auth0Response.scopes.forEach(function(scope) {
        if(clientRequestedScopes.includes(scope.value)) {
          returnScopes.push({
            ...
            consent: 'REQUIRED' //For now- this should be defined in metadata in Auth0 instead.
          })
        }
      });
      returnScopes.push({
