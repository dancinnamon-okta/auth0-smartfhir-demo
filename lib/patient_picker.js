...
module.exports.getHandler = async (requestQueryString) => {
	...
	console.log('Token Passed from Auth0: HI SAMI')
	...
	var tenant = get(verifiedAuth0Data, 'body.tenant'); 
	console.log("THIS IS THE TENANT", tenant); 
	...
	if(getScopes(scope).includes('moyae_ehr')){
		return { 
			statusCode: 200,
			body: nunjucks.render('automatic_ehr_scope_selection.html', {
				scopes: scope, 
	...
	else if(getScopes(scope).includes('launch/patient')) {
		try {
...
module.exports.postHandler = async (requestBodyString) => {
  ...
  return await locationRedirect(requestBodyString); 
}


async function locationRedirect(requestBodyString){
	console.log("this is is what we passed into location Redirect"); 
	const consentBody = querystring.parse(requestBodyString)

	console.log("this is consentBody", consentBody); 
  ...
  const returnUrl = process.env.GATEWAY_URL + '/continue?' +
                    'state=' + auth0State +
                    '&token=' + jwt + '&tenant=' + tenant

  console.log("this is returnUrl", returnUrl); 
  ...
  console.log("this is what we sending back", returnObj)					
	return returnObj; 
}

//This is used by the patient picker to pull application information.
function get_application_data(client_id) {
  console.log('Retrieving Application data from Auth0.', process.env.AUTH0_API_CLIENTID); 
  ...
  console.log("firing after constructor. ", client_id)
	let promise = new Promise(function(resolve, reject) {
...
async function get_scope_data(clientRequestedScopes) {
  ...
  const defaultScopes = [
    ...
    {
      name: 'offline_access',
      displayName: 'Offline_Access: This will allow Moyae to refresh a token automatically'
    }
  ]
  var auth0 = new ManagementClient({
    domain: process.env.AUTH0_ORG,
