...
module.exports.tokenHandler = async (tokenRequestBody, tokenRequestHeaders) => {
	...
	console.log('Calling real /token endpoint at Auth0. TokenRequestBody: ', tokenRequestBody)
	console.log('TokenRequestHeaders', tokenRequestHeaders)
		...
		try {
			const auth0Response = await axios({
				...
				'headers': {
					...
					'User-Agent': get(tokenRequestHeaders, 'User-Agent', 'axios')
				},
				'data': formData
			})
...
async function update_return_claims(jwt_payload, response_body) {
			...
			if(claim === 'scope'){
				...
				if(scopeArray.includes("launch")){
					...
					if(scopeArray.includes("fhirUser" && get(userAppMetadata, 'fhirUser'))){
						// we should 
						response_body['fhirUser'] = userAppMetadata.fhirUser
					}
				}
	...
	console.log("this is userAppMetadata", userAppMetadata); 
	response_body['tenant'] = get(jwt_payload, 'tenant', 'no-tenant'); 
...
function get_auth0_token_request(requestBody, requestHeaders, tokenEndpoint) {
	...
	console.log("this is requestBody", JSON.stringify(requestBody, null, 2)); 
	console.log("this is requestHeaders", JSON.stringify(requestHeaders, null, 2)); 
	...
	if(!requestBody.grant_type){
		// grant type could be hidden in body instead. 

		const parsed = JSON.parse(JSON.stringify(requestBody, null, 2));
		const data = JSON.parse(Object.keys(parsed)[0]);
		formBody = 'grant_type=' + data.grant_type;
